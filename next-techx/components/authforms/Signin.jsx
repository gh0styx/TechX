  ...
  const LoginViaGoogle = async () => {
    ...
    try {
      const ResUserExists = await fetch(
        "https://techx-server.tech:443/auth/CheckUserExists", // <<----- Проверяет есть ли пользователь в базе.
        {
          method: "POST",
      ...
      if (existing_user) {
        try {
          const reply_token = await fetch(
            "https://techx-server.tech:443/session/GenerateToken", //   <<------ Создадим токен.
            {
              method: "POST",
              headers: { "Content-Type": "application/json" },
              body: JSON.stringify({ email: google_auth_email }),
          ...
          const CreateSessionResponse = await fetch(
            "https://techx-server.tech:443/session/CreateSession",
            {
              method: "POST",
              headers: {
    ...
    try {
      ...
      if (existing_user) {
        ...
        const ResProofPass = await fetch(
          "https://techx-server.tech:443/auth/ProofPass", //  <<------ Проверяем пароль пользователя.
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (success) {
          try {
            ...
            const CreateSessionResponse = await fetch(
              "https://techx-server.tech:443/session/CreateSession", // <<----- Создадим пользователю сессию.
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
